...
def load_model(net,
    ...
    if len(pths) == 0 and 'best.pth' not in os.listdir(model_dir):
        return 0
    if epoch == -1:
        if 'best.pth' in os.listdir(model_dir):
            pth = 'best'
        else:
            pth = max(pths)
    else:
...
def save_model(net, optim, scheduler, recorder, model_dir, epoch, last=False, best=False):
